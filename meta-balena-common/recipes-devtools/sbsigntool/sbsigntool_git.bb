...
do_configure:prepend() {
    # Point QEMU to the target sysroot so it finds /lib/ld-linux-<arch>.so.1
    export QEMU_LD_PREFIX="${STAGING_DIR_TARGET}"

    # Add the recipe-sysroot to the library path for the native probes
    ...
    if [ ! -e ${S}/lib/ccan ]; then
        ...
        # It will now use qemu-<arch> (which Bitbake provides)
