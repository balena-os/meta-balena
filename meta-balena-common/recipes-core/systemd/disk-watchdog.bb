...
do_install() {
    ...
    # Substitute paths in boot check script
    sed -i -e 's|@DISK_WD_BOOT_DIR@|${DISK_WD_BOOT_DIR}|g' \
           ${WORKDIR}/disk-watchdog-boot-check

    install -d ${D}${systemd_unitdir}/system
    install -m 0644 ${WORKDIR}/disk-watchdogd.service ${D}${systemd_unitdir}/system/
    install -m 0755 ${WORKDIR}/disk-watchdog-boot-check ${D}${bindir}/
}
