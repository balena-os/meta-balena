...
#     "packages": {
#       "pkg-name": {
#         "categories": ["...", ...],# all categories found for package drivers
#         "in_essential_category": true/false,
#         "interfaces": [...]
#       }
#     },
...
def parse_whences(d, whence_paths):
    ...
    def normalize_whence_path(token):
        ...
        # Normalize to filesystem-style relative paths for matching.
        token = token.strip()
            ...
            for line in f:
                ...
                if line.startswith("Driver:"):
                    ...
                    if line.startswith("File:") or line.startswith("RawFile:"):
                        whence_map[current_driver].append(
                            normalize_whence_path(line.split(':', 1)[1])
                        )
                    elif line.startswith("Link:"):
                        for lf in [normalize_whence_path(p) for p in line.split(':', 1)[1].split('->')]:
...
def _collect_canonical_files_from_dir(directory, firmware_root_absolute):
    import os
    canonical_paths = []
...
def _expand_firmware_pattern(pattern, image_dir, nonarch):
    ...
    for root in possible_roots:
        if not _pattern_under_fw_root(pattern, root):
            continue
        full_pattern = os.path.normpath(os.path.join(image_dir, pattern_normalized))

        # Some FILES entries point to a directory path rather than a glob.
        # In that case collect all contained firmware files recursively.
        if os.path.isdir(full_pattern):
            return sorted(set(_collect_canonical_files_from_dir(full_pattern, firmware_root_absolute)))

        # For glob patterns (including nested globs), expand against image_dir.
        if any(ch in pattern_normalized for ch in ['*', '?', '[']):
            canonical_paths = []
            for filepath in glob.glob(full_pattern, recursive=True):
                if os.path.isdir(filepath):
                    # Some globs (e.g. ".../amphion/*") match directories.
                    # Recurse to collect nested firmware files.
                    canonical_paths.extend(
                        _collect_canonical_files_from_dir(filepath, firmware_root_absolute)
                    )
                elif os.path.isfile(filepath):
                    relative_path = os.path.relpath(filepath, firmware_root_absolute)
                    canonical_paths.append(canonical_firmware_path(relative_path))
            return sorted(set(canonical_paths))

        # Exact file path.
        if os.path.isfile(full_pattern):
            relative_path = os.path.relpath(full_pattern, firmware_root_absolute)
            return [canonical_firmware_path(relative_path)]
...
def resolve_package_metadata(pkg, pkg_files_var, image_dir, nonarch, known_files, driver_categories, fw_feature_map):
    """Build metadata for a single package.
    Returns (firmware_entries, package_entry_or_none)."""
    pkg_drivers, fw_entries = process_package_files(
        pkg, pkg_files_var, image_dir, nonarch, known_files
    )
    if not pkg_drivers:
        return fw_entries, None

    categories = resolve_categories(pkg_drivers, driver_categories, pkg)
    pkg_interfaces = get_package_interfaces(pkg, fw_feature_map)
    if pkg_interfaces:
        bb.note(
            f"[fw-sort] pkg={pkg} categories={','.join(categories)} interfaces={','.join(pkg_interfaces)}"
        )

    pkg_entry = {
        "categories": categories,
        "in_essential_category": is_in_essential_category(categories),
        "interfaces": pkg_interfaces
    }
    return fw_entries, pkg_entry

def resolve_categories(pkg_drivers, driver_categories, pkg):
    """Resolve all package categories from its drivers.
    Fails on first uncategorized driver."""
    pkg_categories = set()
...
python do_firmware_sort() {
    ...
    bb.note(f"[fw-sort] PN={d.getVar('PN')} MACHINE={d.getVar('MACHINE')}")
    bb.note(f"[fw-sort] MACHINE_FEATURES={d.getVar('MACHINE_FEATURES') or ''}")
    ...
    for pkg in packages_var:
        ...
        fw_entries, pkg_entry = resolve_package_metadata(
            pkg,
            pkg_files_var,
            image_dir,
            nonarch,
