...
PACKAGES = "${PN}-engine ${PN}-fs ${PN}-logging ${PN}-time ${PN}-tpm2 ${PN}-config"

do_install() {
    install -d ${D}${libexecdir}
    install -m 0775 \
        ${WORKDIR}/os-helpers-engine \
...
FILES:${PN}-engine = "${libexecdir}/os-helpers-engine"
