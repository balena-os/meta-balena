...
do_test() {
    ...
    JQ="${JQ}" \
    CONF_DIR=${S_UNPACK}/tmp \
    CONFIG_PATH=${S_UNPACK}/test-input.json \
    CACHED_CONFIG_PATH=${S_UNPACK}/tmp/test-cache.json \
    UNITS_DIR=${S_UNPACK}/tmp\
    STAGING_DIR=${STAGING_DIR_NATIVE} \
    /bin/sh "${S_UNPACK}/os-config-json"
    for i in 1 2 3 4 5; do
        if [ -f "${S_UNPACK}/tmp/unit$i.json" ]; then
            cksum1=$(md5sum "${S_UNPACK}/tmp/unit$i.json" | cut -d " " -f1)
            cksum2=$(md5sum "${S_UNPACK}/test-output$i.json" | cut -d " " -f1)
    ...
    tmpfile=$(mktemp)
    "${JQ}" '.key_integer=10' "${S_UNPACK}/test-input.json" > "${S_UNPACK}/${tmpfile}"
    JQ="${JQ}" \
    CONF_DIR=${S_UNPACK}/tmp \
    CONFIG_PATH=${S_UNPACK}/${tmpfile} \
    CACHED_CONFIG_PATH=${S_UNPACK}/tmp/test-cache.json \
    UNITS_DIR=${S_UNPACK}/tmp\
    STAGING_DIR=${STAGING_DIR_NATIVE} \
    /bin/sh "${S_UNPACK}/os-config-json"
    cksum1=$(md5sum "${S_UNPACK}/tmp/unit5.json" | cut -d " " -f1)
    cksum2=$(md5sum "${S_UNPACK}/test-output7.json" | cut -d " " -f1)
    if [ "${cksum1}" != "${cksum2}" ]; then
        bbfatal "[MODIFICATION] os-config-json: Unexpected output"
    fi

    # Test removal of value from configuration unit
    "${JQ}" 'del(.key_object.one)' "${S_UNPACK}/test-input.json" > "${S_UNPACK}/${tmpfile}"
    JQ="${JQ}" \
    CONF_DIR=${S_UNPACK}/tmp \
    CONFIG_PATH=${S_UNPACK}/${tmpfile} \
    CACHED_CONFIG_PATH=${S_UNPACK}/tmp/test-cache.json \
    ...
    STAGING_DIR=${STAGING_DIR_NATIVE} \
    /bin/sh "${S_UNPACK}/os-config-json"
    cksum1=$(md5sum "${S_UNPACK}/tmp/unit1.json" | cut -d " " -f1)
    cksum2=$(md5sum "${S_UNPACK}/test-output6.json" | cut -d " " -f1)
    if [ "${cksum1}" != "${cksum2}" ]; then
        bbfatal "[REMOVAL] os-config-json: Unexpected output"
    fi

    # Test removal of all values from configuration unit
    "${JQ}" 'del(.key_nested.key_nested_child2_nested.key_child2_nested."0") | del(.key_object.one)' "${S_UNPACK}/test-input.json" > "${S_UNPACK}/${tmpfile}"
    JQ="${JQ}" \
    CONF_DIR=${S_UNPACK}/tmp \
    CONFIG_PATH=${S_UNPACK}/${tmpfile} \
    CACHED_CONFIG_PATH=${S_UNPACK}/tmp/test-cache.json \
    ...
    STAGING_DIR=${STAGING_DIR_NATIVE} \
    /bin/sh "${S_UNPACK}/os-config-json"
    if [ -f "${S_UNPACK}/tmp/unit1.json" ]; then
        contents=$(cat "${S_UNPACK}/tmp/unit1.json")
        if [ "${contents}" != "{}" ]; then
...
do_install() {
    install -d ${D}${sbindir}
    install -m 0755 ${S_UNPACK}/os-config-json ${D}${sbindir}/
    install -m 0755 ${S_UNPACK}/gen-conf-unit ${D}${sbindir}/
    install -d ${D}${sysconfdir}/systemd
    install -d ${S_UNPACK}/tmp
    install -c -m 0644 ${S_UNPACK}/unit-conf.json ${STAGING_DIR_TARGET}
    parse_conf_to_units "${S_UNPACK}/unit-conf.json" "${D}/${sysconfdir}/systemd"
    parse_conf_to_units "${S_UNPACK}/test-conf.json" "${S_UNPACK}/tmp"
}
