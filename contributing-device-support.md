...
* because the board support repository needs to be located in balenaOS github organization, once the above steps have been done, 
contact balena in order to have the repository moved / mirrored and the build system jobs created ([2](https://github.com/balena-os/meta-balena/blob/master/contributing-device-support.md#2-contact-balena)),
* send a Pull Request for a hardware contract which describes the boards's capabilities ([3](https://github.com/balena-os/meta-balena/blob/master/contributing-device-support.md#3-hardware-contract)),
...
## 1. Board Support Repository Breakout

The following documentation walks you through the steps of creating such a Yocto package. Because of the substantial difference between the hardware of many boards, this document provides general directions, 
...
The `layers` directory contains the git submodules of the yocto layers used in the build process. The BSP git submodule(s) used should be publicly available repository.
...
- [poky][poky] at the version/revision required by the board BSP (this fork must be used so as to not be rate limited by the yoctoproject git when doing lots of builds)
...
### `recipes-kernel/linux directory`
 ...
 Shall contain a `.bbappend` to the kernel recipe used by the respective board. This kernel `.bbappend` must "inherit kernel-balena" in order to add the necessary kernel configs for using with balenaOS
...
When you have completed the development of the yocto board support repository as detailed in the previous step, please contact balena for finishing the process of having your board available in the balena dashboard.  
...
it can be hosted either as a public repository, for everyone to access, or it can be hosted privately with access only to selected users.

## 3. Hardware contract

Having a board supported by balena also means to have a hardware contract describing that device type.  
...
Note that public / private device type visibility mentioned here is independent to the github repository visibility (you can choose to have any combination of these two).  
...
For private device types, balena will do necessary changes when supplied with the hardware contract.  

## 4. Maintaining the repository and OS updates
...
These automatic pull requests get merged if the CI builds succeed so care must be taken from time to time to check that your code is still compatible with latest meta-balena releases.  

Maintaining / pushing updates to the code other than meta-balena updates needs to be done through pull requests in the appropriate board repository in balena-os github org and will get reviewed by balena prior to getting merged.
