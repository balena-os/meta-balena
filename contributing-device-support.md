...
There is a [sample repo](https://github.com/balena-os/balena-board-template) which we encourage you to use as a starting base for your repository.
...
The `layers` directory contains the git submodules of the yocto layers used in the build process. The BSP git submodule(s) used should be a publicly available repository.
...
### `recipes-kernel/linux directory`
 ...
 Shall contain a `.bbappend` to the kernel recipe used by the respective board. This kernel `.bbappend` must "inherit kernel-balena" in order to add the necessary kernel configs that is used for balenaOS
...
Maintaining / pushing updates to the code other than meta-balena updates need to be done through pull requests in the appropriate board repository in balena-os GitHub org and will be reviewed by balena prior to merging.
