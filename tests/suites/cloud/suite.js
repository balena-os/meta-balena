...
module.exports = {
	...
	run: async function (test) {
		...
		this.suite.teardown.register(() => {
			return Promise.resolve(this.cloud.removeSSHKey(this.balena.sshKey.label));
		});
		...
		const config = await this.cloud.balena.models.os.getConfig(
			this.balena.application,
			{
		...
		config.installer = {
			...
			migrate: {
				force: this.suite.options.balenaOS.config.installerForceMigration,
			},
		};

		if (
			this.workerContract.workerType === `qemu` &&
			config.installer.migrate.force
		) {
