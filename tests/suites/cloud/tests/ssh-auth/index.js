...
const setConfig = async (test, that, target, key, value) => {
	return test.test(`Update or delete ${key} in config.json`, (t) =>
		t
			.resolves(
				that.waitForServiceState('config-json.service', 'inactive', target),
	...
	tests: [
		{
			...
			run: async function (test) {
				return setConfig(test, this, this.balena.uuid, 'developmentMode', false)
					...
					.then(async () => {
						return exec(`ssh-add ${this.context.get().sshKeyPath}`).then(() => {
					...
					.then(async () => {
						...
						await this.worker
							.executeCommandInHostOS('true', this.link, retryOptions)
							.then(() => {
								throw new Error(
									'SSH authentication passed when it should have failed',
								);
							})
							.catch((err) => {
								return test.match(
					...
					.then(async () => {
						return setConfig(test, this, this.balena.uuid, 'os.sshKeys', [
							this.context.get().sshKey.pubKey.trim(),
						]);
					})
					.then(async () => {
						let result;
					...
					.then(async () => {
						let result;
						let config = {};
						const ip = await this.worker.getDutIp(this.link);
						await this.utils.waitUntil(
							async () => {
								if (!this.worker.directConnect) {
									...
									result = await this.worker.executeCommandInWorker(
										`sh -c "ssh -p 22222 -i /tmp/id -o StrictHostKeyChecking=no ${this.worker.username}@${ip} echo -n pass"`,
									);
									return result;
								} else {
									config = {
									...
									result = await this.utils.executeCommandOverSSH(
										`echo -n pass`,
										config,
									);
			...
			run: async function (test) {
				...
				return setConfig(test, this, this.balena.uuid, 'developmentMode', true)
					...
					.then(async () => {
						return test.throws(
							function () {
								this.worker.executeCommandInHostOS('echo -n pass', this.link);
							},
							{},
