	...
	tests: [
		{
			...
			run: async function (test) {
				...
				await exec(
					`echo "#comment" >> ${this.appPath}/containerA/Dockerfile.template`,
				);
				test.comment(`Pushing release...`);

				const secondCommit = await this.cloud.pushReleaseToApp(
				...
				const usedDeltas = await this.cloud.checkLogsContain(
					this.balena.uuid,
			...
			run: async function (test) {
				...
				await this.utils.waitUntil(
					async () => {
						...
						services.current_services[this.appServiceName].forEach(
							(service) => {
								if (
