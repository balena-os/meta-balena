...
const PIEEPROM_SCRIPT = '/usr/libexec/pieeprom-update.sh';
...
// Run the pieeprom-update.sh script and return its output
const runPieepromUpdate = async (context, link, args = '') => {
  return (await context
    .get()
    .worker.executeCommandInHostOS(
      `${PIEEPROM_SCRIPT} ${args} 2>&1 || true`,
      link,
    )).trim();
};
...
module.exports = {
  ...
  run: async function (test) {
    ...
    test.comment('Running pieeprom-update.sh with force-flashrom...');
    const flashromOutput = await runPieepromUpdate(context, link, 'pieeprom.upd force-flashrom');
    test.comment(`Script output: ${flashromOutput}`);
    test.ok(
      flashromOutput.includes('Flashrom forced via argument'),
      'Script should log forced flashrom message',
    );
    test.ok(
    ...
    const selfUpdateOutput = await runPieepromUpdate(context, link);
