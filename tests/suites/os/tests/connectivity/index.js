			...
			tests: ['wired', 'wireless'].map((adaptor) => {
				return {
					...
					run: async function (test) {
						const connection = adaptor === 'wireless' ? 'balena-wifi' : 'Wired';
						return this.worker
							...
							.then((iface) => {
								...
								test.comment(
									`Attempting to connect to ${URL_TEST} over interface ${iface}`,
								);
		...
		},
		{
			...
			tests: ['socks5', 'http-connect'].map((proxy) => {
				return {
					...
					run: async function (test) {
						const getProxyContainerID = async () => {
							return this.worker.executeCommandInHostOS(
								['balena', 'ps', '-qf', 'name=proxy'],
								this.link,
							);
						};
						const getRedsocksUid = async () => {
							return this.worker.executeCommandInHostOS(
								`id -u redsocks`,
								this.link,
							);
						...
						return Promise.resolve(this.worker.ip(this.link))
							.then((ip) => {
								return getProxyContainerID().then((containerId) => {
									...
									if (!containerId) {
										...
										return getRedsocksUid().then((redsocksUid) => {
											...
											try {
												const composeContents = fs.readFileSync(
													composeFile,
													'utf8',
												);
												const updatedCompose = composeContents.replace(
													/REDSOCKS_UID/g,
													redsocksUid.toString(),
												);
												fs.writeFileSync(composeFile, updatedCompose, 'utf8');
												test.comment(
													'Updated docker-compose.yml with redsocks uid ' +
														redsocksUid,
												);
											} catch (err) {
												test.comment(
													`Failed to update docker-compose.yml - ` + err,
												);
											}

											return this.worker
												.pushContainerToDUT(ip, __dirname, 'proxy')
												.then((state) => {
													test.comment(state);
												});
										});
