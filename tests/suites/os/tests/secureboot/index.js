	...
	tests: [
		{
			...
			run: async function (test) {
				...
				const securebootSupported = await this.worker
					.executeCommandInHostOS(
						[
							'ls',
							securebootEfiVarPath,
							'&>/dev/null',
							'&&',
							'echo',
							'true',
				...
				const securebootEnabled = await this.worker
					.executeCommandInHostOS(
						[
							'od',
							'--address-radix=n',
				...
				const waitForSerialOutput = async (
					pattern,
					slice = 0,
					retries = 60,
					delay = 1000,
				...
				const testFullDiskEncryption = async () =>
					Promise.all(
						[
							{ pattern: '/^\\/mnt\\/boot$/', name: 'Boot partition' },
							{
								pattern: '/^\\/mnt\\/sysroot\\/active$/',
								name: 'Root partition',
							},
					...
					return this.worker
						.executeCommandInHostOS(
							[
								'sed',
								'-i',
								's/lockdown=integrity//',
								'/mnt/efi/EFI/BOOT/grub.cfg',
							],
							this.link,
						)
						.then(() => this.worker.executeCommandInHostOS('reboot', this.link))
