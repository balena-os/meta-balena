const blockNTP = async (test, that, target) => {
	return test.test(`Blocking NTP by stopping dnsmasq.service`, (t) =>
		t
			...
			.then(() => {
				return t.resolves(
					that.systemd.waitForServiceState('chronyd.service', 'active', target),
...
const restoreNTP = async (test, that, target) => {
	return test.test(`Unblocking NTP by restarting dnsmasq.service`, (t) =>
		t
			...
			.then(() => {
				return t.resolves(
					that.systemd.waitForServiceState('dnsmasq.service', 'active', target),
					...
					.then(() => {
						return test.resolves(
							this.utils.waitUntil(
								async () => {
									const regex =
										/NTP time lost synchronization - restarting chronyd/;
									return this.worker
										.executeCommandInHostOS(
											`journalctl -u chronyd.service`,
											this.link,
