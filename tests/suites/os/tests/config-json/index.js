	...
	tests: [
		{
			...
			run: async function (test) {
				...
				return this.systemd
					.writeConfigJsonProp(test, 'hostname', hostname, context.link)
					.then(() => {
					...
					.then(() => {
						return this.systemd.writeConfigJsonProp(
							test,
							'hostname',
							null,
							context.link,
		...
		},
		{
			...
			run: async function (test) {
				...
				return this.systemd
					.writeConfigJsonProp(test, 'ntpServers', ntpServer(), context.link)
					.then(() => {
						return test.resolves(
							this.utils.waitUntil(
								async () => {
									...
									return context.worker
										.executeCommandInHostOS(
											[
												...
												'|',
												'awk',
												'"/\\[connectivity\\]/{flag=1;next}/\\[/{flag=0}flag"',
											].join(' '),
											context.link,
										)
										.then((config) => {
		...
		},
		{
			...
			run: async function (test) {
				...
				return this.systemd
					.writeConfigJsonProp(
						test,
						'os.network.wifi.randomMacAddressScan',
						true,
						context.link,
					)
					.then(() => {
									...
									return context.worker
										.executeCommandInHostOS(
											[
												...
												'|',
												'awk',
												'"/\\[device\\]/{flag=1;next}/\\[/{flag=0}flag"',
											].join(' '),
											context.link,
										)
										.then((config) => {
		...
		},
		{
			...
			run: async function (test) {
				...
				return this.systemd
					.writeConfigJsonProp(test, 'os.udevRules', rule, context.link)
					.then(() => {
