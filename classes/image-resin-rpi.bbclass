...
IMAGE_CMD_resin-noobs () {
	...
	echo "dwc_otg.lpm_enable=0 console=null root=@ROOT@ rootfstype=ext4 elevator=deadline rootwait quiet" > ${BOOT_WORK}/cmdline.txt
...
IMAGE_CMD_resin-noobs-dev () {
	...
	## Overwrite the cmdline.txt
	echo "dwc_otg.lpm_enable=0 console=ttyAMA0,115200 kgdboc=ttyAMA0,115200 root=@ROOT@ rootfstype=ext4 rootwait debug elevator=deadline" > ${BOOT_WORK}/cmdline.txt
