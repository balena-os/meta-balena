...
No hook is shipped by any extension meta-balena builds. Kernel override activation is the runtime's, not a hook's; see [Kernel override extensions](#kernel-override-extensions).

## Kernel override extensions

A hostapp extension can replace the running kernel rather than only adding files to the root filesystem. Such an extension carries a kernel image under `/boot`, plus the matching modules and their `Module.symvers` under `/usr/lib/modules/<release>/`.
...
Installing the extension publishes its kernel under `/mnt/data/boot-by-abi/<kernel-abi-id>`; activating it arms that build id for the next boot. If the armed kernel is missing, if no deployed extension still supplies it, or if the initramfs cannot load it, the boot falls back to the stock kernel shipped with the OS.

The activation path is `balena-extension-runtime`'s, so the code that writes the boot environment ships with the OS that defines its keys:

* `create` fabricates the extension's `/boot` volume and fills it from the image, then runs the extension's own `hooks/create` if it ships one. Nothing is published at this point.
* `start` runs the extension's `hooks/start` if it ships one, then activates. Activation verifies the extension against its `io.balena.image.kernel-abi-id` label, refuses an ABI that boot-time validation has already rejected, symlinks `/mnt/data/boot-by-abi/<abi>` to the kernel image inside the extension's `/boot` volume, using a path relative to the data partition, records whether the VPN was reachable before the change, and writes `kernel_override_abi` into the boot environment. That write comes last, and it is what opens the validation window described below. It clears `kernel_override_trial` in the same block write, so a count left by a window that already closed cannot shorten this one.
* The initramfs `kexec` script reads `kernel_override_abi`, boots `/mnt/data/boot-by-abi/<abi>` when it resolves and falls back to the stock kernel otherwise. The link names the kernel image rather than the volume holding it, so a publication whose kernel has gone reads as a broken link rather than as a directory the boot has to search. It publishes the kernel it loaded on the command line as `balena_kernel_abi=<abi>`, and that token is how the rest of the system knows which kernel is running.

The extension's own hook runs first because the arm has to be the last step that can carry a verdict: a hook that declined after the arm would leave a failed container beside an armed override.

The runtime identifies a kernel override by the same signal it fabricates a volume on, `io.balena.image.kernel-abi-id`. The label is the claim; the shipped bytes verify it, as the regular file directly under the extension's `/boot` whose sha256 equals the label. An extension with no such label activates nothing. An extension carrying the label and no matching file is refused, and a refusal is recorded as the container exiting non-zero, so nothing retries a decision that will not change. An extension whose rootfs carries no `Module.symvers` is refused the same way, and that is the only point where the label and the modules behind it are checked against each other: mobynit answers the claim query off the container store without mounting, so it reads the label and never sees the tree.

A machine condition is not a refusal. `/mnt/state` not being mounted, an unwritable boot environment, or a fabricated volume that is not where the data partition expects it fails the `start` call instead, leaving the container created so the caller can deploy it again.

A pending purge suppresses the override for that boot. mobynit reports no claimed ABIs while the reset marker is missing from the data partition, because it is not going to mount the extension overlays either, so the initramfs falls back to the stock kernel and the boot does not run a kernel whose modules are absent. The validator sees the same empty answer and forgets the ABI on that boot.

### Validating a kernel override

An armed override is on trial until a boot ratifies it. Four boot environment keys carry that state:
...
* `kernel_override_trial`: how many boots the armed override has had without a verdict. The initramfs writes it, `start` clears it when it arms, and the validator clears it when the window closes.

A boot whose live value matches the running slot's committed value is an ordinary boot and costs one read of the boot environment. Anything else is a pending verdict, reached down one of two paths.

Inside a HUP, `rollback-health` owns the verdict, and it delegates both outcomes to the manager. Healthchecks passing run `balena-extension-manager hup commit`, which records the running ABI as the running slot's committed override and clears the trial count. Healthchecks failing run `balena-extension-manager hup reject`, which restores the other slot's committed override and relays the rejection to the next boot through `kernel_override_abi_rejected`, so the kernel and the rootfs move together. Neither failure stops the rollback or the commit hooks: the manager's exit is warned about and the update path continues.

Outside a HUP, `extension-rollback.service` owns it. The unit runs `balena-extension-manager validate` on every boot, because an override can be armed with no update in progress and so leaves no breadcrumb to gate on. It stands aside while a HUP is in flight, then settles, runs the same healthchecks through `rollback-tests`, and either commits the ABI for the running slot or rejects it. Each verdict is a single write of the boot environment block, taken under the same lock the runtime arms under.

A kernel that never reaches those healthchecks is bounded by the trial count instead. The stage-2 initramfs counts each boot of an armed override the running slot has not committed, and appends `panic=30` to that boot so a kernel that dies before userspace resets the board rather than hanging. After three such boots stage 2 stops offering the armed kernel and boots the slot's committed override, or the stock kernel when the slot has none. The validator then reads the spent count next to an arm that is not running, records the ABI in `/mnt/state/override-rejected` so nothing re-arms the same kernel bytes, and writes `by=boots` and `boots=3` into `/mnt/state/override-health-triggered`. Rebuilding the extension with different kernel bytes gives a different ABI and arms normally.

The count is boots without a verdict, not boots that failed. An operator reboot, a power cut and a userspace crash loop each spend an attempt, which is what bounds a crash loop as well as a dead kernel, and is the same rule the root filesystem `bootcount` has always applied. What it does not reach is a kernel that boots, brings up networking and then sits there degraded without rebooting: nothing counts boots that do not happen, and outside an update window `rollback-health` does not run at all. That case needs manual intervention.
...
|---|---|---|---|---|
| `balena-extension-manager validate`, from `extension-rollback.service` | every boot, before the armed override is judged, with the engine possibly down | the `hostapp` claim query linked into the manager, the one mobynit wraps, reading the container store off the disk | no container claims this ABI | boot environment keys, the `boot-by-abi` symlink |
