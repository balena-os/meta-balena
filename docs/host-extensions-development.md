...
An extension must not ship `/usr/libexec/os-helpers-*`. Those files are sourced by every host process that uses them, `rollback-health` and the host OS update hooks included.
